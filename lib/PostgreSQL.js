...
export default class Postgres {
	constructor(auth) {
		...
		return this.pool.query('SHOW CLIENT_ENCODING')
			.then(result => {
				G.debug('数据库', '连接', `✔  ~[数据库用户]~{${this.user}} ~[客户端编码]~{${result.rows[0].client_encoding}}`);
	...
	async end() {
		...
		return G.debug('数据库', '关闭', `✔  ~[数据库用户]~{${this.user}}`);
	}

	format(sql, ...params) { return formatSQL(sql, params); }

	async pick() { return new PostgresClient(await this.pool.connect()); }
